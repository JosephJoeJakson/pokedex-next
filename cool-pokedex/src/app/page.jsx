...
{isModalOpen && selectedPokemon && (
  ...
  <div className={styles.filterBar}>
    ...
    {types.map((type, index) => (
      <label key={index}
             className={`${styles.filterLabel} ${selectedTypes.includes(type) ? styles.active : selectedTypes.length >= 2 && !selectedTypes.includes(type) ? styles.dim : ''}`}>
        <input
          type="checkbox"
...
<div className={styles.list}>
  {pokemons.map((pokemon, index) => (
    <div key={index} className={`${pokemon.types[0]} ${styles.pokemonCard}`} onClick={() => selectPokemon(pokemon)} style={{ backgroundImage: `url(${pokemon.types[0]}.png)` }}>
      ...
      <div className={styles.pokemonListInfo}>
        ...
        <ul className={styles.pokemonListTypes}>
          {pokemon.types.map((type, typeIndex) => (
            <li className={`${type} type`} key={typeIndex}>{type}</li>  // Changed <p> to <li> for semantic correctness
          ))}
        </ul>
      </div>
    </div>
